...
class BasePlugin:
    ...
    def onStart(self):
        ...
        # Check and update the ASR params
            ...
            if (connex == 0):
                ...
                if self.ModeAuto:
                    if self.PLUGINstarteddtime + timedelta(minutes = 10) <= now:
                        if self.PresenceTH and self.ASRconnexchangedtime + timedelta(minutes = 3) <= now and self.controlsettime + timedelta(minutes = 1) <= now:
                        Devices[10].Update(nValue = 0,sValue = (Devices[6].sValue))  # Thermostat setpoint = AC setpoint
                        self.setpoint = float(Devices[6].sValue)
        ...
        if self.powerOn :
            if self.ModeAuto and Devices[1].nValue != 0 :
                if self.PresenceTH :
                    ...
                    if self.intemp < (float(Devices[10].sValue) - ((self.deltamax + 25) / 10)):
                        self.setpoint = 30
                ...
                self.setpoint = float(Devices[6].sValue)
                Domoticz.Log("MANUALmode or ASR Not Connected - used setpoint is : " + str(self.setpoint))
