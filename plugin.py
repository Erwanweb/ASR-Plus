    ...
    def checkconnexion(self):
        ...
        try:
            ...
            if response.status == 200:
                ...
                # Server SR index
                if not Devices[2].sValue == str(id):
                ...
                if (connex == 0):
                    ...
                    # AC Setpoint
                    Devices[6].Update(nValue = stemp,sValue = (str(stemp)+".0"))
                    if self.ModeManual:
                        self.setpoint = float(Devices[6].sValue)
                        if not Devices[10].sValue == str(self.setpoint):
                            Devices[10].Update(nValue = 0,sValue = (Devices[6].sValue))  # Thermostat setpoint = AC setpoint
...
def ESPcommandAPI(APICall):
    ...
    try:
        ...
        response = request.urlopen(req)
        if response.status == 200:
            Domoticz.Debug("Climodom Command API Sent -- OK")
        else:
            Domoticz.Error("Climodom Command API: http error = {}".format(response.status))
    except:
        Domoticz.Error("Climodom seems not connected - Command not sent")
    return resultJson
