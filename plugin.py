...
class BasePlugin:
    ...
    def __init__(self):
        ...
        self.presenceoffdelay = 1  # time between last detection before turning presence OFF
        return
    ...
    def onMessage(self,Connection,Data):
        ...
        if (Connection == self.httpConnControlInfo):
            ...
            if (connex == 0):
                ...
                if self.ModeManual:
                    Devices[10].Update(nValue = self.setpoint,sValue = (Devices[6].sValue))  # Thermostat setpoint = AC setpoint
    ...
    def onCommand(self,Unit,Command,Level,Color):
        ...
        if (Unit == 3):
            ...
            Devices[6].Update(nValue = self.setpoint,sValue = Devices[6].sValue)
            Devices[7].Update(nValue = self.powerOn,sValue = Devices[7].sValue)
            Devices[9].Update(nValue = self.powerOn,sValue = Devices[9].sValue)
            Devices[10].Update(nValue = self.setpoint,sValue = Devices[10].sValue)
        ...
        if (Unit == 9):
            ...
            if (Devices[9].sValue == "20"):
                ...
                Devices[6].Update(nValue = self.setpoint,sValue = (Devices[10].sValue))  # AC setpoint = Thermostat setpoint
        ...
        if (Unit == 10):
            Devices[10].Update(nValue = self.setpoint,sValue = str(Level))
            if self.ModeManual:
                Devices[6].Update(nValue = self.setpoint,sValue = (Devices[10].sValue))  # AC setpoint = Thermostat setpoint
    ...
    def onHeartbeat(self):
        ...
        # Check if the used setpoint is ok
        if self.powerOn :
            if self.ModeAuto :
                if self.PresenceTH :
        ...
        if Parameters["Mode3"] == "":
            ...
            if self.PresenceDetected:
                ...
                if Devices[8].nValue == 0:
                    ...
                    Domoticz.Log("No presence detected in the past 30 seconds...")
                    Devices[8].Update(nValue = 0,sValue = Devices[8].sValue)
                    self.Presence = False
                    self.presencechangedtime = datetime.now()
