...
<plugin key="AC-ASRplus" name="AC Aircon Smart Remote PLUS" author="MrErwan" version="0.1.1" externallink="https://github.com/Erwanweb/ASR-Plus.git">
    ...
    <params>
        ...
        <param field="Mode5" label="Day/Night Activator, Pause On delay, Pause Off delay, Presence On delay, Presence Off delay (all in minutes), reducted T, Delta max fanspeed (in degree)" width="200px" required="true" default="0,1,1,2,45,3"/>
...
class BasePlugin:
    ...
    def __init__(self):
        self.debug = False
        self.setpoint = 21.0
        self.deltamax = 0.5  # allowed deltamax from setpoint ofr high level airfan
    ...
    def onStart(self):
        ...
        if len(params) == 7:
            ...
            self.deltamax = CheckParam("delta max fan",params[6],0.5)
        else:
