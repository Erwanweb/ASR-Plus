    ...
    def onMessage(self,Connection,Data):
        ...
        if (Connection == self.httpConnControlInfo):
            ...
            if (connex == 0):
                ...
                #if self.ModeAuto:
                    #if self.PLUGINstarteddtime + timedelta(minutes = 20) <= now:
                        #if self.PresenceTH and self.ASRconnexchangedtime + timedelta(minutes = 3) and self.controlsettime + timedelta(minutes = 1) <= now:
                        #Devices[10].Update(nValue = 0,sValue = (Devices[6].sValue))  # Thermostat setpoint = AC setpoint
                        #self.setpoint = float(Devices[6].sValue)
        ...
        if self.powerOn :
            if self.ModeAuto and Devices[1].nValue != 0 :
                if self.PresenceTH :
                    ...
                    if Devices[6].sValue != str(self.setpoint) :
                    ...
                    if Devices[6].sValue != str(self.setpoint) :
                        Devices[6].Update(nValue = self.setpoint,sValue = str(self.setpoint))  # AC setpoint = Thermostat setpoint reducted in limit of range
                        self.httpConnSetControl.Connect()
                    else :
                        if not Devices[5].sValue == "10":
                        Devices[5].Update(nValue = self.powerOn,sValue = "10")  # AC Fan Speed Auto
