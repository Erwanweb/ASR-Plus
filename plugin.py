...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        # Check if the used mode is ok
        if (Devices[9].nValue != 0 and Devices[9].sValue == "20"):
            self.ModeAuto = True
            self.ModeManual = False

        elif (Devices[9].nValue != 0 and Devices[9].sValue == "30"):
            self.ModeAuto = False
            self.ModeManual = True
        ...
        if self.powerOn :
            if self.ModeAuto :
                if self.PresenceTH :
                    ...
                    if self.intemp < self.setpoint - (self.deltamax/10):
