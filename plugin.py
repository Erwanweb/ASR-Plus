...
Version:    0.0.1: alpha
Version:    0.1.1: beta
Version:    0.2.1: connexion mode changed
"""
"""
<plugin key="AC-ASRplus" name="AC Aircon Smart Remote PLUS" author="MrErwan" version="0.1.1" externallink="https://github.com/Erwanweb/ASR-Plus.git">
    ...
    <params>
        ...
        <param field="Mode5" label="Day/Night Activator, Pause On delay, Pause Off delay, Presence On delay, Presence Off delay (all in minutes), reducted T(in degree), Delta max fanspeed (in in tenth of degre)" width="200px" required="true" default="0,1,1,2,45,3,0.5"/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if len(params) == 7:
            ...
            self.deltamax = CheckParam("delta max fan",params[6],5)
        else:
            ...
            if self.ModeAuto and Devices[1].nValue != 0 :
                if self.PresenceTH :
                    ...
                    if self.intemp < (float(Devices[10].sValue) - ((self.deltamax / 10) + (self.deltamax / 20))):
                        self.setpoint = 30
                        Domoticz.Log("AUTOMode - used setpoint is Max 30 and fan speed Max because room temp is lower more than delta min from setpoint : " + str(self.setpoint))
                        if not Devices[6].sValue == str(self.setpoint):
                            Devices[6].Update(nValue = 0,sValue = "30")  # AC setpoint = max setpoint
                    ...
                    Devices[6].Update(nValue = 0,sValue = str(self.setpoint))  # AC setpoint = Thermostat setpoint reducted in limit of range
                ...
                Domoticz.Log("MANUAL mode or ASR Not Connected - used setpoint is : " + str(self.setpoint))
    ...
    def checkconnexion(self):
        ...
        try:
            ...
            if response.status == 200:
                ...
                id = -1 # on met id a -1 pour valider que l'on a bien trouve la telecommande et connex a 0 pour etre sur de la connexion
                connex = 0

                # on parcourt toute la liste des telecommandes

                for remoteObject in resultJson["Remotes"]:
                ...
                if id > -1:
                    ...
                    Domoticz.Debug(
                        "Power: " + onoff + "; Mode: " + mode + "; FanSpeed:" + fanspeed + "; AC Set temp: " + str(
                            stemp) + "; Wmode: " + winmode)
                ...
                if (connex == 0):
                    ...
                    # Control
                    if self.powerOn:
                        if self.ModeAuto and not (Devices[9].sValue == "20"):
                            Devices[9].Update(nValue = self.powerOn,sValue = "20")
                        elif self.ModeManual and not (Devices[9].sValue == "30"):
                    ...
                    if self.ModeManual:
                        ...
                        self.setpoint = float(Devices[6].sValue)
                    if self.ModeAuto:
                        if self.PLUGINstarteddtime + timedelta(minutes = 10) <= now:
                            if self.PresenceTH and self.ASRconnexchangedtime + timedelta(
                                    minutes = 3) <= now and self.controlsettime + timedelta(minutes = 1) <= now:
                                Devices[10].Update(nValue = 0,
                                                   sValue = (Devices[6].sValue))  # Thermostat setpoint = AC setpoint
                            self.setpoint = float(Devices[6].sValue)
                ...
                Domoticz.Error("Climodom connect API: http error = {}".format(response.status))

        except:
            Domoticz.Error("Climodom seems not connected")
            Devices[1].Update(nValue = 0,sValue = "0")
...
def ESPconnectAPI(APICall):
    ...
    resultJson = None
    url = "http://{}/api_chunghopserver?status=".format(Parameters["Username"], parse.quote(APICall))
    Domoticz.Debug("Calling Climodom Connect API: {}".format(url))
    try:
        req = request.Request(url)
        response = request.urlopen(req)
        if response.status == 200:
            resultJson = json.loads(response.read().decode('utf-8'))
            Domoticz.Debug("Climodom Connected -- OK")
        else:
            Domoticz.Error("Climodom Connect API: http error = {}".format(response.status))
